...
#include <v11_cat2_detector/Cat2DetectorParamsConfig.h>
...
private:
    ...
    ros::Publisher square_pos_pub_;

    geometry_msgs::Point line_pos_;
    ros::Publisher line_pos_pub_;   
//vertikal line baru
    geometry_msgs::Point line_pos_ver_;
    ros::Publisher line_pos_ver_pub_;   
    ...
    std::string square_config_path;
    v11_cat2_detector::Cat2DetectorParamsConfig config_;
    dynamic_reconfigure::Server<v11_cat2_detector::Cat2DetectorParamsConfig> param_server_;
    dynamic_reconfigure::Server<v11_cat2_detector::Cat2DetectorParamsConfig>::CallbackType param_cb_;
    void paramCallback(v11_cat2_detector::Cat2DetectorParamsConfig &_config, uint32_t level);
    ...
    void squareRefCallback(const sensor_msgs::ImageConstPtr &_msg);
    
    image_transport::Subscriber it_bf_sub_;
    cv_bridge::CvImagePtr cv_bf_ptr_sub_;
    cv::Mat square_ref_;
    cv::MatND square_ref_hist_;
    ...
    cv::Mat cvtMulti(const cv::Mat &_square_ref);

    cv::Mat in_hsv_;
    cv::Mat thresh_image_;

    cv::Mat segmentColor(cv::Mat &_segmented_green, cv::Mat &_inv_segmented_green, cv::Mat &_segmented_blue, cv::Mat &_segmented_white);
